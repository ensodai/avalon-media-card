...
@Factory
class UpdateProwlarrSettingsUseCase(private val adminRepository: AdminRepository) {
    suspend operator fun invoke(request: UpdateProwlarrSettingsRequest): Result<Unit> {
        return try {
            val response = adminRepository.updateProwlarrSettings(request)
            if (response.success) {
                Result.success(Unit)
            } else {

...
class TestProwlarrConnectionUseCase(
    ...
    suspend fun execute(userId: Uuid?, url: String, apiKey: String): ActionResult {
        validationStore.setValidationStatus(userId, "prowlarr_url", ValidationStatus.Pending, context.i18n.t("status.checking"))
        
        if (url.isBlank()) {
            validationStore.setValidationStatus(userId, "prowlarr_url", ValidationStatus.Error, context.i18n.t("status.enter_url"))
            return ActionResult.NoOp
        }
        
        val result = ProwlarrClient.testConnection(httpClient, url, apiKey)
        return when (result) {
            is ConnectionResult.Success -> {
                validationStore.setValidationStatus(userId, "prowlarr_url", ValidationStatus.Success, context.i18n.t("status.connected"))
                ActionResult.ShowNotification(context.i18n.t("notification.prowlarr_success"), "success")
            }
            is ConnectionResult.AuthError -> {
                val msg = result.message ?: context.i18n.t("status.auth_error")
                validationStore.setValidationStatus(userId, "prowlarr_url", ValidationStatus.Error, msg)
                ActionResult.NoOp
            }
